...
import { Hospital, LogOut, ClipboardList, Archive, Save } from "lucide-react";
...
if (filterMode === "today")       { start = today; end = today; }
  ...
  const handleDownload = () => {
    const data = (filteredEntries.length ? filteredEntries : rows.filter((r) => r.uhid || r.claimId || r.opdNo || r.patientName))
      .map((row, index) => ({
        "S.No.": index + 1,
        UHID: row.uhid || "",
        ...
        "OPD Date": row.opdDate || "",
        "Upload Date": row.uploadDate || "",
        Hospital: row.hospital || "",
        "Prepare By": row.prepareBy || "",
        Remarks: row.remarks || "",
        "Added By": row.addedBy || "",
      }));
    const ws = XLSX.utils.json_to_sheet(data);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "OPD Logs");
    XLSX.writeFile(wb, `Sangi_OPD_${filterMode}_${today}.xlsx`);
  };
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100vh", background: "var(--bg)", color: "var(--text)", fontFamily: "var(--ui-font-sans)", overflow: "hidden" }}>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .ogrid-cell:focus { outline: 2px solid ${accent}; outline-offset: -2px; background: var(--accent-soft) !important; z-index: 2; position: relative; }
        .ogrid-cell { transition: background 0.1s; font-family: var(--ui-font-sans); }
        .ogrid-cell:hover { background: var(--surface-2) !important; }
        .otab-btn:hover { color: ${accent} !important; background: var(--accent-soft) !important; }
        .ofilter-chip:hover { border-color: ${accent} !important; color: ${accent} !important; background: var(--accent-soft) !important; }
      ...
      <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 28px", height: 62, borderBottom: "2px solid var(--border)", background: "var(--card)", flexShrink: 0, boxShadow: "var(--shadow-sm)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <div style={{ width: 38, height: 38, borderRadius: 10, background: "var(--hdr-chip-bg)", border: "1px solid var(--hdr-chip-border)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, color: "var(--hdr-text)", fontWeight: 700, boxShadow: "var(--shadow-sm)" }}><Hospital size={18} strokeWidth={2.1} /></div>
          <div>
            ...
            <div style={{ fontSize: 16, fontWeight: 700, color: "var(--text)", letterSpacing: "-0.3px" }}>OPD Department</div>
          </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          {[{ label: "Today", val: todayCount, col: "var(--success)" }, { label: "This Week", val: weekCount, col: accent }, { label: "This Month", val: monthCount, col: "var(--warning)" }].map(({ label, val, col }) => (
          ...
          {currentUser && (
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginLeft: 8, paddingLeft: 14, borderLeft: "2px solid var(--border)" }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 9, color: "var(--text-muted)", letterSpacing: "1.5px", textTransform: "uppercase", fontFamily: "var(--ui-font-sans)" }}>OPD Staff</div>
              </div>
              <button onClick={onLogout} style={{ marginLeft: 6, padding: "5px 14px", borderRadius: 8, background: "var(--danger-soft)", border: "1.5px solid var(--danger-border)", color: "var(--danger)", fontSize: 10, cursor: "pointer", fontFamily: "var(--ui-font-sans)", fontWeight: 700, display:"inline-flex", alignItems:"center", gap:6 }}><LogOut size={12} strokeWidth={2.1} /> Logout</button>
            </div>
          )}
        </div>
      </header>
...
{syncError && <div style={{ fontSize: 10, color: "var(--danger)", fontWeight: 600 }}>{syncError}</div>}
              {hasUnsaved && <div style={{ fontSize: 10, color: accent, animation: "opulse 2s infinite", fontFamily: "var(--ui-font-sans)", fontWeight: 600 }}>● Unsaved changes</div>}
              ...
              <button className="oaction-btn" onClick={() => addRows(5)} style={{ padding: "6px 16px", borderRadius: 8, fontSize: 11, fontFamily: "var(--ui-font-sans)", cursor: "pointer", background: "var(--surface)", border: "1.5px solid var(--border)", color: "var(--text-muted)", fontWeight: 600 }}>+ 5 Rows</button>
              <button className="oaction-btn" onClick={handleSave} style={{ padding: "6px 20px", borderRadius: 8, fontSize: 12, fontFamily: "var(--ui-font-sans)", cursor: "pointer", background: `linear-gradient(135deg, ${accent}, var(--accent-strong))`, border: "none", color: "var(--text-on-accent)", fontWeight: 700, boxShadow: "var(--shadow-sm)", display:"inline-flex", alignItems:"center", gap:6 }}><Save size={13} strokeWidth={2} /> Save</button>
              <button className="oaction-btn" onClick={handleDownload} style={{ padding: "6px 16px", borderRadius: 8, fontSize: 11, fontFamily: "var(--ui-font-sans)", cursor: "pointer", background: "var(--surface-2)", border: "1.5px solid var(--border)", color: "var(--info)", fontWeight: 600 }}>↓ Export Today</button>
            </>
          )}
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflow: "hidden", display: "flex", flexDirection: "column" }}>
        ...
        {viewTab === "entry" && (
          <div style={{ flex: 1, overflow: "auto", padding: "20px 28px" }} className="ofade-in">
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16, padding: "14px 20px", background: "var(--card)", border: `1.5px solid ${accent}40`, borderRadius: 12, borderLeft: `5px solid ${accent}`, boxShadow: "0 2px 8px #f8717110" }}>
              <div>
                <div style={{ fontSize: 13, fontWeight: 700, color: "var(--text)" }}>Today's OPD Log — {today}</div>
              ...
              <table style={{ borderCollapse: "collapse", width: "100%", fontSize: 12 }}>
                <thead>
                  <tr style={{ background: "#ffe4e6" }}>
                    {COLUMNS.map(col => (
                      <th key={col.key} style={{ padding: "11px 14px", textAlign: "left", fontSize: 9, letterSpacing: "2px", color: "#be123c", textTransform: "uppercase", borderBottom: "2px solid #fecdd3", whiteSpace: "nowrap", minWidth: col.width, fontFamily: "var(--ui-font-sans)", fontWeight: 700, borderRight: "1px solid #fecdd3" }}>
                        {col.label}
                      </th>
                    ))}
                      ...
                      <tr key={row.id} style={{ background: filled ? "#fff1f2" : "#ffffff", borderBottom: "1px solid #ffe4e6" }}>
                        {COLUMNS.map(col => (
                          <td key={col.key} style={{ padding: 0, borderRight: "1px solid #ffe4e6" }}>
                            {col.readOnly ? (
                              <div style={{ padding: "9px 14px", color: "#fca5a5", fontSize: 11, userSelect: "none", fontFamily: "var(--ui-font-sans)" }}>{row[col.key]}</div>
                            ) : (
                              <input
                                ...
                                style={{ width: "100%", padding: "10px 14px", background: "transparent", border: "none", color: col.key === "patientName" ? "#1c0a0a" : "#374151", fontSize: 11, fontFamily: "var(--ui-font-sans)", outline: "none", minWidth: col.width, cursor: "text", fontWeight: col.key === "patientName" ? 600 : 400 }}
        ...
        {viewTab === "records" && (
          <div style={{ flex: 1, overflow: "auto", padding: "20px 28px" }} className="ofade-in">
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20, padding: "14px 20px", background: "var(--card)", border: "1.5px solid #fecdd3", borderRadius: 12, flexWrap: "wrap", boxShadow: "0 2px 8px #f8717108" }}>
              <span style={{ fontSize: 9, color: "#fca5a5", letterSpacing: "2.5px", textTransform: "uppercase", marginRight: 4, fontFamily: "var(--ui-font-sans)", fontWeight: 600 }}>Period:</span>
              {[{ id: "today", label: "Today" }, { id: "week", label: "This Week" }, { id: "month", label: "This Month" }, { id: "year", label: "This Year" }, { id: "custom", label: "Custom" }].map(f => (
                <button key={f.id} className="ofilter-chip" onClick={() => setFilterMode(f.id)}
                  style={{ padding: "6px 16px", borderRadius: 20, fontSize: 11, fontFamily: "var(--ui-font-sans)", cursor: "pointer", background: filterMode === f.id ? `${accent}15` : "#f9fafb", border: `1.5px solid ${filterMode === f.id ? accent : "#fecdd3"}`, color: filterMode === f.id ? accent : "#6b7280", fontWeight: filterMode === f.id ? 700 : 600, transition: "all 0.15s" }}>
                  {f.label}
                </button>
              ))}
              {filterMode === "custom" && (
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginLeft: 8 }}>
                  <input type="date" value={customStart} onChange={e => setCustomStart(e.target.value)} style={{ background: "var(--bg)", border: "1.5px solid #fecdd3", color: "var(--text)", padding: "6px 12px", borderRadius: 8, fontSize: 11, fontFamily: "var(--ui-font-sans)", outline: "none" }} />
                  <span style={{ color: "#9ca3af", fontSize: 10 }}>to</span>
                  <input type="date" value={customEnd} onChange={e => setCustomEnd(e.target.value)} style={{ background: "var(--bg)", border: "1.5px solid #fecdd3", color: "var(--text)", padding: "6px 12px", borderRadius: 8, fontSize: 11, fontFamily: "var(--ui-font-sans)", outline: "none" }} />
                </div>
              )}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14, marginBottom: 20 }}>
              {[
                ...
                <div key={label} style={{ background: "var(--card)", border: "1.5px solid #fecdd3", borderTop: `4px solid ${col}`, borderRadius: 12, padding: "16px 18px", position: "relative", overflow: "hidden", boxShadow: "0 2px 8px #f8717108" }}>
            ...
            {filteredEntries.length === 0 ? (
              <div style={{ textAlign: "center", padding: 60, color: "#fecdd3", fontSize: 12, letterSpacing: "3px", background: "var(--card)", border: "1.5px solid #fecdd3", borderRadius: 12, fontFamily: "var(--ui-font-sans)", fontWeight: 700 }}>NO RECORDS FOUND FOR THIS PERIOD</div>
            ) : (
                ...
                <div style={{ overflowX: "auto" }}>
                  <table style={{ borderCollapse: "collapse", width: "100%", fontSize: 12 }}>
                    <thead>
                      <tr style={{ background: "#ffe4e6" }}>
                        {COLUMNS.map(col => (
                          <th key={col.key} style={{ padding: "11px 14px", textAlign: "left", fontSize: 9, letterSpacing: "2px", color: "#be123c", textTransform: "uppercase", borderBottom: "2px solid #fecdd3", whiteSpace: "nowrap", fontFamily: "var(--ui-font-sans)", fontWeight: 700, borderRight: "1px solid #fecdd3" }}>{col.label}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {filteredEntries.map((row, i) => (
                        <tr key={row.id || i} style={{ borderBottom: "1px solid #ffe4e6" }}>
                          <td style={{ padding: "10px 14px", color: "#fca5a5", fontSize: 10, borderRight: "1px solid #ffe4e6", fontFamily: "var(--ui-font-sans)" }}>{i + 1}</td>
                          <td style={{ padding: "10px 14px", color: "#0ea5e9", fontFamily: "var(--ui-font-sans)", borderRight: "1px solid #ffe4e6", fontWeight: 600 }}>{row.uhid || "—"}</td>
                          <td style={{ padding: "10px 14px", color: accent, fontFamily: "var(--ui-font-sans)", borderRight: "1px solid #ffe4e6", fontWeight: 600 }}>{row.claimId || "—"}</td>
                          <td style={{ padding: "10px 14px", color: "#374151", fontFamily: "var(--ui-font-sans)", borderRight: "1px solid #ffe4e6" }}>{row.opdNo || "—"}</td>
                          <td style={{ padding: "10px 14px", color: "var(--text)", fontWeight: 700, borderRight: "1px solid #ffe4e6" }}>{row.patientName || "—"}</td>
                          <td style={{ padding: "10px 14px", color: "#6b7280", fontSize: 11, borderRight: "1px solid #ffe4e6", fontFamily: "var(--ui-font-sans)" }}>{row.opdDate || "—"}</td>
