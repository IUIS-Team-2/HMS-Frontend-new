...
export default function ManagementAdminDashboard({ currentUser, db, locId, onLogout }) {
  ...
  const handlePrintSummary = p => {
    ...
    printWithAuth(`${BASE_URL}/patients/${p.uhid}/admissions/${admNo}/dynamic-summary/print/?type=${encodeURIComponent(printType)}`).catch(e => toast.error(e.message||"Print failed."));
  };
  ...
  const addTemplateReport = (p,templateName) => { const t=LAB_TEMPLATES[templateName]; if(!t) return; setPatientReports(prev=>{ const existing=prev[p.uhid]||{}; if(existing[templateName]) return prev; const tests=t.tests.map(tt=>({id:crypto.randomUUID(),name:tt.name||"",value:"",unit:tt.unit||"",refRange:tt.refRange||"",status:"Normal"})); return{...prev,[p.uhid]:{...existing,[templateName]:{id:Date.now(),reportName:templateName,report_date:new Date().toISOString().slice(0,10),reportType:"Haematology",orderedBy:"",amount:0,remarks:t.defaultRemarks||"",findings:"",impression:"",tests,saved:false}}}; }); };
  ...
  const printRepReport = (p,rn) => printWithAuth(`${BASE_URL}/patients/${p.uhid}/admissions/${String(resolveAdmNo(p))}/lab-reports/print/?report=${encodeURIComponent(rn)}`).catch(e => toast.error(e.message||"Print failed."));
