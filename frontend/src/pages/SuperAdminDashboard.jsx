...
function BillPrintModal({ p, onClose }) {
  ...
  const billDate = new Date().toLocaleDateString("en-IN", { day: "2-digit", month: "2-digit", year: "numeric" }) + " " + new Date().toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit", hour12: false }) + " HRS";

  const handlePrint = () => {
    const printContent = document.getElementById("bill-print-content").innerHTML;
    const win = window.open("", "_blank", "width=900,height=700");
    win.document.write(`<!DOCTYPE html><html><head><title>Bill - ${p.name}</title><style>
      ...
      .header-grid{display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:10px}
      .info-box{border:1px solid #000;padding:6px 10px}
      .total-row td{font-weight:bold;background:#f9f9f9}
      @media print{@page{size:A4;margin:10mm}}
    </style></head><body>${printContent}<script>window.onload=()=>{window.print()}<\/script></body></html>`);
    win.document.close();
  };
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.85)", zIndex: 4000, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ background: T.surface, borderRadius: 16, width: "100%", maxWidth: 900, maxHeight: "94vh", overflow: "hidden", display: "flex", flexDirection: "column", border: `1px solid ${T.border}`, boxShadow: "0 32px 100px rgba(0,0,0,.8)" }}>
        ...
        <div style={{ overflowY: "auto", padding: 24 }}>
          <div id="bill-print-content" style={{ background: "#fff", color: "#000", fontFamily: "Arial,sans-serif", fontSize: 12, padding: 24, borderRadius: 8 }}>
            {/* Top: Logo + Hospital Info */}
...
function PatientModal({ p, onClose }) {
  ...
  const handleLoadDocument = async () => {
    ...
    } catch (err) { toast.error("Failed to load document template."); }
    setDocLoading(false);
  };
  ...
  const handleVitalsUpdate = (index, vKey, val) => {
    const newSections = [...docTemplate.sections];
    newSections[index] = { ...newSections[index], value: { ...newSections[index].value, [vKey]: val } };
    setDocTemplate({ ...docTemplate, sections: newSections });
  };
...
/* ══════════════════════════════════════════════════════════════
   BILLING TAB — with bill-format view + print
══════════════════════════════════════════════════════════════ */
...
/* ══════════════════════════════════════════════════════════════
   MEDICAL HISTORY TAB — uses MedicalHistoryPage component
   Prefills data from backend per patient
══════════════════════════════════════════════════════════════ */
function MedicalTab({ all }) {
  ...
  const handleOpenPatient = async (p) => {
    ...
    try {
      ...
      // Prefill from existing medHistory if available
    ...
    return (
      <div>
        ...
        <MedicalHistoryPage
          data={medData}
          setData={setMedData}
          onSave={handleSaveMedData}
          onSkip={() => setSelectedPatient(null)}
          patient={selectedPatient._patient}
          discharge={selectedPatient._admission?.discharge}
          locId={selectedPatient._branch}
        />
      </div>
    );
  }
            ...
            {rows.map((p, i) => {
              ...
              return (
                <tr key={i} style={{ borderBottom: `1px solid ${T.border}`, background: i % 2 === 0 ? T.card : T.surface }}>
                  ...
                  <td style={{ padding: "9px 12px" }}>
                    <button onClick={() => handleOpenPatient(p)} style={{ padding: "4px 10px", borderRadius: 6, background: T.laxmi + "20", color: T.laxmi, border: `1px solid ${T.laxmi}40`, fontSize: 11, fontWeight: 700, cursor: "pointer" }}>
...
/* ══════════════════════════════════════════════════════════════
   LAB REPORTS TAB — fetches from backend
══════════════════════════════════════════════════════════════ */
...
function LabReportsTab({ all }) {
  ...
  const handlePrintReport = (report) => {
    ...
    const branch = branchInfo[selectedPatient?._branch] || branchInfo.laxmi;
    ...
    win.document.write(`<!DOCTYPE html><html><head><title>Lab Report</title><style>
      ...
      table{width:100%;border-collapse:collapse}th{background:#f0f0f0;border:1px solid #ccc;padding:6px 10px;font-size:11px}
      @media print{@page{size:A4;margin:10mm}}
    </style></head><body>
    <div style="display:flex;justify-content:space-between;border-bottom:2px solid #000;padding-bottom:10px;margin-bottom:12px">
      <div><div style="font-size:28px;font-weight:900;color:#1a5b8c;letter-spacing:2px">SANGi</div><div style="font-size:13px;font-weight:700;color:#d93838;letter-spacing:4px">HOSPITAL</div></div>
      <div style="text-align:right;font-size:11px;color:#555"><div>${branch.name}</div><div>${branch.address}</div><div>Ph: ${branch.phone}</div></div>
    </div>
    ...
    <div style="display:flex;justify-content:space-between;margin-top:40px">
      ...
      <div style="text-align:right"><div style="font-weight:700">Dr. Rakesh Koul</div><div style="font-size:11px;color:#555">(M.B.B.S. DCP)</div><div style="font-weight:700;border-top:1px solid #000;padding-top:5px">PATHOLOGIST</div></div>
    </div>
      ...
      <div style={{ minHeight: "100vh", background: T.bg, fontFamily: "'Segoe UI',system-ui,sans-serif" }}>
        ...
        <div style={{ width: 228, minHeight: "100vh", background: T.sidebar, display: "flex", flexDirection: "column", position: "fixed", top: 0, left: 0, zIndex: 50, borderRight: `1px solid ${T.border}` }}>
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "6px 6px" }}>
            {NAV.map((n, i) => {
              if (n.section) return (
                <div key={i} style={{ fontSize: 10, color: T.dimmer, fontWeight: 700, textTransform: "uppercase", letterSpacing: ".1em", padding: "12px 10px 4px" }}>{n.section}</div>
              );
