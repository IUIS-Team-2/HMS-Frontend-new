...
export default function ManagementAdminDashboard({ currentUser, db, locId, onLogout }) {
  ...
  const renderHome = () => {
    ...
    const stats = [
      { label:"Branch Patients",    val:locationPatients.length, col:accent,    icon:Users, sub:"All records" },
      { label:"Total Admissions",   val:allAdmissions.length,    col:"#22d3ee", icon:ClipboardList, sub:"All time" },
      { label:"Currently Admitted", val:currentlyAdmitted,       col:"#34d399", icon:Hospital, sub:"Active" },
      { label:"Discharged",         val:discharged,              col:"#8b949e", icon:DoorOpen, sub:"Completed" },
      { label:"Total Tasks",        val:tasks.length,            col:"#818cf8", icon:CheckSquare, sub:"All tasks" },
      { label:"Pending Tasks",      val:pendingTasks,            col:"#f59e0b", icon:Clock3, sub:"Awaiting action" },
      { label:"Urgent Tasks",       val:urgentTasks,             col:"#f87171", icon:AlertTriangle, sub:"Need attention" },
      { label:"Departments",        val:departments.length,      col:"#34d399", icon:Building2, sub:"Active depts" },
    ];
    return (
      <div>
        ...
        <aside className="hms-sb" style={{ width:sbWidth }}>
          ...
          <nav className="hms-nav-wrap">
            ...
            {NAV.map(item=>{
              ...
              return (<div key={item.id} className={`hms-nav-item${activeTab===item.id?" active":""}`}
                ...
                <span className="hms-nav-icon" style={{ display:"inline-flex", alignItems:"center" }}>
