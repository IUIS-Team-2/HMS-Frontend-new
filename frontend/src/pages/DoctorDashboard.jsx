...
export default function DoctorDashboard({ currentUser, onLogout }) {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", height:"100vh", background:"var(--bg)", color:"var(--text)", fontFamily:"var(--ui-font-sans)", overflow:"hidden" }}>
      ...
      {/* ── Topbar ── */}
      <header style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 26px", height:64, borderBottom:"1px solid var(--border)", background:"var(--surface)", flexShrink:0 }}>
        <div style={{ display:"flex", alignItems:"center", gap:14 }}>
          ...
          <div>
            <div style={{ fontSize:10, fontWeight:600, color:"var(--text-dim)", textTransform:"uppercase", letterSpacing:"2.5px" }}>Sangi Hospital</div>
            <div style={{ fontSize:18, fontWeight:700, color:"var(--text)", letterSpacing:"-0.4px", marginTop:1 }}>Doctor Dashboard</div>
          </div>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          ...
          {[
            ...
            <div key={lbl} style={{ display:"flex", alignItems:"center", gap:6, padding:"6px 14px", borderRadius:20, background:"var(--surface-2)", border:"1px solid var(--border-strong)", fontSize:12 }}>
          ...
          {currentUser && (
            <div style={{ display:"flex", alignItems:"center", gap:9, marginLeft:8, paddingLeft:14, borderLeft:"1px solid var(--border)" }}>
              ...
              <div>
                <div style={{ fontSize:13, color:"var(--text)", fontWeight:600 }}>{currentUser.name}</div>
              ...
              style={{ marginLeft:4, padding:"6px 14px", borderRadius:7, background:"var(--danger-soft)", border:"1px solid var(--danger-border)", color:"var(--danger)", fontSize:11, cursor:"pointer", fontFamily:"inherit", fontWeight:600 }}>
                Logout
              </button>
            </div>
          )}
      ...
      {/* ── Sub-nav ── */}
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 26px", height:52, borderBottom:"1px solid var(--border)", background:"var(--surface)", flexShrink:0 }}>
        ...
        {viewTab === "entry" && (
          <div style={{ display:"flex", gap:8, alignItems:"center" }}>
            {hasUnsaved && <span style={{ fontSize:12, color:"var(--warning)", fontWeight:600, animation:"blink 2s infinite" }}>● Unsaved changes</span>}
            {savedAt && !hasUnsaved && <span style={{ fontSize:12, color:"var(--success)", fontWeight:500 }}>✓ Saved {savedAt}</span>}

            <button onClick={() => addRows(5)} className="btn_hover"
              style={{ padding:"7px 16px", borderRadius:7, fontSize:12, fontFamily:"inherit", cursor:"pointer", background:"var(--surface-2)", border:"1px solid var(--border-strong)", color:"var(--text-muted)", fontWeight:500 }}>
              + 5 Rows
            </button>

            <button onClick={handleSave} className="btn_hover"
              style={{ padding:"7px 20px", borderRadius:7, fontSize:13, fontFamily:"inherit", cursor:"pointer", background:"var(--border)", border:`1px solid ${COLOR}40`, color:COLOR, fontWeight:600 }}>
              💾 Save Draft
            </button>
      ...
      {submittedToday && viewTab === "entry" && (
        <div style={{ display:"flex", alignItems:"center", gap:10, padding:"8px 26px", background:"var(--success-soft)", borderBottom:"1px solid var(--success-border)", flexShrink:0 }}>
          <span style={{ fontSize:16 }}>✅</span>
          <span style={{ fontSize:12, color:"var(--success)", fontWeight:600 }}>Today&apos;s records have been submitted to HOD & Admin Management</span>
      ...
      <div style={{ flex:1, overflow:"hidden", display:"flex", flexDirection:"column" }}>
        ...
        {viewTab === "entry" && (
          <div style={{ flex:1, overflow:"auto", padding:"20px 26px" }} className="dc_fade">
            ...
            {/* Banner */}
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:18, padding:"14px 22px", background:"var(--surface)", border:`1px solid ${COLOR}35`, borderRadius:12, borderLeft:`4px solid ${COLOR}` }}>
              <div>
                        ...
                        <tr key={row.id} className="dc_tr" style={{ background: filled ? "var(--surface-2)" : "transparent", borderBottom:"1px solid var(--border)" }}>
                            ...
                            <td key={col.key} style={{ padding:0, borderRight:"1px solid var(--border)" }}>
                              {col.readOnly ? (
                                ...
                                <input
                                  ...
                                  style={{ width:"100%", padding:"9px 14px", background:"transparent", border:"none", color:"var(--info)", fontSize:12, fontFamily:"'JetBrains Mono',monospace", outline:"none", minWidth:col.width, textAlign:"center", fontWeight:600 }}
                                  placeholder="—"
                                />
                              ) : (
                                <input
                                  ...
                                  style={{
                                    ...
                                    color: col.key === "patientName" ? "var(--text)"
                                         : col.key === "claimId"     ? COLOR
                                         : col.key === "uhid"        ? "var(--text-mid)"
                                         : col.key === "overView"    ? "var(--accent)"
                                         : "var(--text-mid)",
                          ...
                          <td style={{ padding:"0 6px", textAlign:"center" }}>
                            <button className="dc_row_rm" onClick={() => removeRow(row.id)}
                              style={{ background:"none", border:"none", color:"var(--danger)", cursor:"pointer", fontSize:14, padding:"2px 5px" }}>
                              ✕
                            </button>
                          </td>
                        </tr>
            ...
            {filledToday > 0 && !submittedToday && (
              <div style={{ marginTop:16, padding:"14px 20px", background:"var(--warning-soft)", border:"1px solid var(--warning-border)", borderRadius:10, display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                <div>
                  ...
                  <div style={{ fontSize:11, color:"var(--text-mid)", marginTop:2 }}>{filledToday} record{filledToday !== 1 ? "s" : ""} ready · HOD & Admin are waiting for today&apos;s summary</div>
                </div>
                <button onClick={handleSubmit} className="btn_hover"
                  style={{ padding:"8px 22px", borderRadius:8, fontSize:13, fontFamily:"inherit", cursor:"pointer", background:COLOR, border:"none", color:"var(--text-on-accent)", fontWeight:700, boxShadow:`0 0 16px ${GLOW}` }}>
                  Submit Now →
                </button>
              </div>
            )}
              ...
              {filterMode === "custom" && (
                <div style={{ display:"flex", alignItems:"center", gap:8, marginLeft:8 }}>
                  <input type="date" value={customStart} onChange={e => setCustomStart(e.target.value)}
                    style={{ background:"var(--bg)", border:"1px solid var(--border-strong)", color:"var(--text-mid)", padding:"6px 10px", borderRadius:7, fontSize:12, fontFamily:"'JetBrains Mono',monospace", outline:"none" }} />
