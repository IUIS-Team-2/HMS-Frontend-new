...
import { LogOut, ClipboardList, Archive, Save } from "lucide-react";
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100vh", background: "var(--bg)", color: "var(--text)", fontFamily: "var(--ui-font-sans)", overflow: "hidden" }}>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .qgrid-cell:focus { outline: 2px solid ${accent}; outline-offset: -2px; background: var(--accent-soft) !important; z-index: 2; position: relative; }
        .qgrid-cell { transition: background 0.1s; font-family: var(--ui-font-sans); }
        .qgrid-cell:hover { background: var(--surface-2) !important; }
        .qtab-btn:hover { color: ${accent} !important; background: var(--accent-soft) !important; }
        .qfilter-chip:hover { border-color: ${accent} !important; color: ${accent} !important; background: var(--accent-soft) !important; }
      ...
      <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 28px", height: 62, borderBottom: "2px solid var(--border)", background: "var(--card)", flexShrink: 0, boxShadow: "var(--shadow-sm)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 16, fontWeight: 700, color: "var(--text)", letterSpacing: "-0.3px" }}>Query Department</div>
          </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          {[{ label: "Today", val: todayCount, col: "var(--success)" }, { label: "This Week", val: weekCount, col: accent }, { label: "This Month", val: monthCount, col: "var(--info)" }].map(({ label, val, col }) => (
          ...
          {currentUser && (
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginLeft: 8, paddingLeft: 14, borderLeft: "2px solid var(--border)" }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 9, color: "var(--text-muted)", letterSpacing: "1.5px", textTransform: "uppercase", fontFamily: "var(--ui-font-sans)" }}>Query Staff</div>
              </div>
...
{syncError && <div style={{ fontSize: 10, color: "var(--danger)", fontWeight: 600 }}>{syncError}</div>}
              {hasUnsaved && <div style={{ fontSize: 10, color: accent, animation: "qpulse 2s infinite", fontFamily: "var(--ui-font-sans)", fontWeight: 600 }}>● Unsaved changes</div>}
              ...
              <button className="qaction-btn" onClick={() => addRows(5)} style={{ padding: "6px 16px", borderRadius: 8, fontSize: 11, fontFamily: "var(--ui-font-sans)", cursor: "pointer", background: "var(--surface)", border: "1.5px solid var(--border)", color: "var(--text-muted)", fontWeight: 600 }}>+ 5 Rows</button>
              <button className="qaction-btn" onClick={handleSave} style={{ padding: "6px 20px", borderRadius: 8, fontSize: 12, fontFamily: "var(--ui-font-sans)", cursor: "pointer", background: `linear-gradient(135deg, ${accent}, var(--accent-strong))`, border: "none", color: "var(--text-on-accent)", fontWeight: 700, boxShadow: "var(--shadow-sm)", display:"inline-flex", alignItems:"center", gap:6 }}><Save size={13} strokeWidth={2} /> Save</button>
              <button className="qaction-btn" onClick={handleDownload} style={{ padding: "6px 16px", borderRadius: 8, fontSize: 11, fontFamily: "var(--ui-font-sans)", cursor: "pointer", background: "var(--surface-2)", border: "1.5px solid var(--border)", color: "var(--info)", fontWeight: 600 }}>↓ Export Today</button>
            </>
          )}
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflow: "hidden", display: "flex", flexDirection: "column" }}>
        ...
        {viewTab === "entry" && (
          <div style={{ flex: 1, overflow: "auto", padding: "20px 28px" }} className="qfade-in">
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16, padding: "14px 20px", background: "var(--card)", border: `1.5px solid ${accent}40`, borderRadius: 12, borderLeft: `5px solid ${accent}`, boxShadow: "0 2px 8px #f59e0b10" }}>
              <div>
                <div style={{ fontSize: 13, fontWeight: 700, color: "var(--text)" }}>Today's Query Log — {today}</div>
            ...
            <div style={{ overflowX: "auto", background: "var(--card)", border: "1.5px solid #fde68a", borderRadius: 12, overflow: "hidden", boxShadow: "0 2px 12px #f59e0b08" }}>
              <table style={{ borderCollapse: "collapse", width: "100%", fontSize: 12 }}>
                <thead>
                  <tr style={{ background: "#fef3c7" }}>
                    {COLUMNS.map(col => (
                      <th key={col.key} style={{ padding: "11px 14px", textAlign: "left", fontSize: 9, letterSpacing: "2px", color: "#b45309", textTransform: "uppercase", borderBottom: "2px solid #fde68a", whiteSpace: "nowrap", minWidth: col.width, fontFamily: "var(--ui-font-sans)", fontWeight: 700, borderRight: "1px solid #fde68a" }}>
                        {col.label}
                      </th>
                    ))}
                      ...
                      <tr key={row.id} style={{ background: filled ? "#fffbeb" : "#ffffff", borderBottom: "1px solid #fef3c7" }}>
                        {COLUMNS.map(col => (
                          <td key={col.key} style={{ padding: 0, borderRight: "1px solid #fef3c7" }}>
                            {col.readOnly ? (
                              <div style={{ padding: "9px 14px", color: "#fbbf24", fontSize: 11, userSelect: "none", fontFamily: "var(--ui-font-sans)" }}>{row[col.key]}</div>
                            ) : (
                              <input
                                id={`cell-${rowIdx}-${col.key}`}
                                className="qgrid-cell"
                                type={col.type || "text"}
                                value={row[col.key] || ""}
                                onChange={e => updateRow(row.id, col.key, e.target.value)}
                                onKeyDown={e => handleKeyDown(e, rowIdx, col.key)}
                                style={{ width: "100%", padding: "10px 14px", background: "transparent", border: "none", color: col.key === "patientName" ? "#1c1917" : "#374151", fontSize: 11, fontFamily: "var(--ui-font-sans)", outline: "none", minWidth: col.width, cursor: "text", fontWeight: col.key === "patientName" ? 600 : 400 }}
        ...
        {viewTab === "records" && (
          <div style={{ flex: 1, overflow: "auto", padding: "20px 28px" }} className="qfade-in">
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20, padding: "14px 20px", background: "var(--card)", border: "1.5px solid #fde68a", borderRadius: 12, flexWrap: "wrap", boxShadow: "0 2px 8px #f59e0b08" }}>
              <span style={{ fontSize: 9, color: "#fbbf24", letterSpacing: "2.5px", textTransform: "uppercase", marginRight: 4, fontFamily: "var(--ui-font-sans)", fontWeight: 600 }}>Period:</span>
              {[{ id: "today", label: "Today" }, { id: "week", label: "This Week" }, { id: "month", label: "This Month" }, { id: "year", label: "This Year" }, { id: "custom", label: "Custom" }].map(f => (
                <button key={f.id} className="qfilter-chip" onClick={() => setFilterMode(f.id)}
                  style={{ padding: "6px 16px", borderRadius: 20, fontSize: 11, fontFamily: "var(--ui-font-sans)", cursor: "pointer", background: filterMode === f.id ? `${accent}15` : "#f9fafb", border: `1.5px solid ${filterMode === f.id ? accent : "#fde68a"}`, color: filterMode === f.id ? accent : "#6b7280", fontWeight: filterMode === f.id ? 700 : 600, transition: "all 0.15s" }}>
                  {f.label}
                </button>
              ))}
              {filterMode === "custom" && (
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginLeft: 8 }}>
                  <input type="date" value={customStart} onChange={e => setCustomStart(e.target.value)} style={{ background: "var(--bg)", border: "1.5px solid #fde68a", color: "var(--text)", padding: "6px 12px", borderRadius: 8, fontSize: 11, fontFamily: "var(--ui-font-sans)", outline: "none" }} />
                  <span style={{ color: "#9ca3af", fontSize: 10 }}>to</span>
                  <input type="date" value={customEnd} onChange={e => setCustomEnd(e.target.value)} style={{ background: "var(--bg)", border: "1.5px solid #fde68a", color: "var(--text)", padding: "6px 12px", borderRadius: 8, fontSize: 11, fontFamily: "var(--ui-font-sans)", outline: "none" }} />
                </div>
              )}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14, marginBottom: 20 }}>
              {[
                ...
                <div key={label} style={{ background: "var(--card)", border: "1.5px solid #fde68a", borderTop: `4px solid ${col}`, borderRadius: 12, padding: "16px 18px", position: "relative", overflow: "hidden", boxShadow: "0 2px 8px #f59e0b08" }}>
            ...
            {filteredEntries.length === 0 ? (
              <div style={{ textAlign: "center", padding: 60, color: "#fde68a", fontSize: 12, letterSpacing: "3px", background: "var(--card)", border: "1.5px solid #fde68a", borderRadius: 12, fontFamily: "var(--ui-font-sans)", fontWeight: 700 }}>NO RECORDS FOUND FOR THIS PERIOD</div>
            ) : (
                ...
                <div style={{ overflowX: "auto" }}>
                  <table style={{ borderCollapse: "collapse", width: "100%", fontSize: 12 }}>
                    <thead>
                      <tr style={{ background: "#fef3c7" }}>
                        {COLUMNS.map(col => (
                          <th key={col.key} style={{ padding: "11px 14px", textAlign: "left", fontSize: 9, letterSpacing: "2px", color: "#b45309", textTransform: "uppercase", borderBottom: "2px solid #fde68a", whiteSpace: "nowrap", fontFamily: "var(--ui-font-sans)", fontWeight: 700, borderRight: "1px solid #fde68a" }}>{col.label}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {filteredEntries.map((row, i) => (
                        <tr key={row.id || i} style={{ borderBottom: "1px solid #fef3c7" }}>
                          <td style={{ padding: "10px 14px", color: "#fbbf24", fontSize: 10, borderRight: "1px solid #fef3c7", fontFamily: "var(--ui-font-sans)" }}>{i + 1}</td>
                          <td style={{ padding: "10px 14px", color: "#0ea5e9", fontFamily: "var(--ui-font-sans)", borderRight: "1px solid #fef3c7", fontWeight: 600 }}>{row.uhid || "—"}</td>
                          <td style={{ padding: "10px 14px", color: accent, fontFamily: "var(--ui-font-sans)", borderRight: "1px solid #fef3c7", fontWeight: 600 }}>{row.claimId || "—"}</td>
                          <td style={{ padding: "10px 14px", color: "var(--text)", fontWeight: 700, borderRight: "1px solid #fef3c7" }}>{row.patientName || "—"}</td>
                          <td style={{ padding: "10px 14px", color: "#6b7280", fontSize: 11, borderRight: "1px solid #fef3c7", fontFamily: "var(--ui-font-sans)" }}>{row.raiseDate || "—"}</td>
                          <td style={{ padding: "10px 14px", color: "#6b7280", fontSize: 11, borderRight: "1px solid #fef3c7", fontFamily: "var(--ui-font-sans)" }}>{row.queryRepDate || "—"}</td>
