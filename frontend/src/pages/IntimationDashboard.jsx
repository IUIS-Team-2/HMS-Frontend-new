...
import { ClipboardList, LogOut, Archive, Save } from "lucide-react";
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100vh", background: "var(--bg)", color: "var(--text)", fontFamily: "var(--ui-font-sans)", overflow: "hidden" }}>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .igrid-cell:focus { outline: 2px solid ${accent}; outline-offset: -2px; background: var(--accent-soft) !important; z-index: 2; position: relative; }
        .igrid-cell { transition: background 0.1s; font-family: var(--ui-font-sans); }
        .igrid-cell:hover { background: var(--surface-2) !important; }
        .itab-btn:hover { color: ${accent} !important; background: var(--accent-soft) !important; }
        .ifilter-chip:hover { border-color: ${accent} !important; color: ${accent} !important; background: var(--accent-soft) !important; }
      ...
      <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 28px", height: 62, borderBottom: "2px solid var(--border)", background: "var(--card)", flexShrink: 0, boxShadow: "var(--shadow-sm)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <div style={{ width: 38, height: 38, borderRadius: 10, background: "var(--hdr-chip-bg)", border: "1px solid var(--hdr-chip-border)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, color: "var(--hdr-text)", fontWeight: 700, boxShadow: "var(--shadow-sm)" }}><ClipboardList size={18} strokeWidth={2.1} /></div>
          <div>
            ...
            <div style={{ fontSize: 16, fontWeight: 700, color: "var(--text)", letterSpacing: "-0.3px" }}>Intimation Department</div>
          </div>
          <div style={{ marginLeft: 10, padding: "4px 14px", borderRadius: 20, background: "var(--accent-soft)", border: `1.5px solid ${accent}`, fontSize: 10, color: accent, letterSpacing: "1px", fontFamily: "var(--ui-font-sans)", fontWeight: 600 }}>{today}</div>
        </div>

        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          {[{ label: "Today", val: todayCount, col: accent }, { label: "This Week", val: weekCount, col: "var(--info)" }, { label: "This Month", val: monthCount, col: "var(--warning)" }].map(({ label, val, col }) => (
          ...
          {currentUser && (
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginLeft: 8, paddingLeft: 14, borderLeft: "2px solid var(--border)" }}>
              <div style={{ width: 32, height: 32, borderRadius: 8, background: "var(--accent-soft)", border: `1.5px solid ${accent}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, color: accent, fontWeight: 700 }}>{currentUser.name?.[0] || "I"}</div>
              <div>
                <div style={{ fontSize: 12, color: "var(--text)", fontWeight: 700 }}>{currentUser.name}</div>
                <div style={{ fontSize: 9, color: "var(--text-muted)", letterSpacing: "1.5px", textTransform: "uppercase", fontFamily: "var(--ui-font-sans)" }}>Intimation Staff</div>
              </div>
              <button onClick={onLogout} style={{ marginLeft: 6, padding: "5px 14px", borderRadius: 8, background: "var(--danger-soft)", border: "1.5px solid var(--danger-border)", color: "var(--danger)", fontSize: 10, cursor: "pointer", fontFamily: "var(--ui-font-sans)", fontWeight: 700, transition: "all 0.15s", display:"inline-flex", alignItems:"center", gap:6 }}><LogOut size={12} strokeWidth={2.1} /> Logout</button>
            </div>
          )}
        </div>
      </header>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 28px", height: 48, borderBottom: "1.5px solid var(--border)", background: "var(--bg)", flexShrink: 0 }}>
        <div style={{ display: "flex", gap: 4 }}>
          {[{ id: "entry", label: "Daily Entry", Icon: ClipboardList }, { id: "records", label: "Records", Icon: Archive }].map(tab => (
...
{syncError && <div style={{ fontSize: 10, color: "var(--danger)", fontWeight: 600 }}>{syncError}</div>}
              {hasUnsaved && <div style={{ fontSize: 10, color: "var(--warning)", animation: "ipulse 2s infinite", fontFamily: "var(--ui-font-sans)", fontWeight: 600 }}>● Unsaved changes</div>}
              {savedAt && !hasUnsaved && <div style={{ fontSize: 10, color: accent, fontFamily: "var(--ui-font-sans)", fontWeight: 600 }}>✓ Saved at {savedAt}</div>}
              <button className="iaction-btn" onClick={() => addRows(5)} style={{ padding: "6px 16px", borderRadius: 8, fontSize: 11, fontFamily: "var(--ui-font-sans)", cursor: "pointer", background: "var(--surface)", border: "1.5px solid var(--border)", color: "var(--text-muted)", fontWeight: 600, transition: "all 0.15s" }}>+ 5 Rows</button>
              <button className="iaction-btn" onClick={handleSave} style={{ padding: "6px 20px", borderRadius: 8, fontSize: 12, fontFamily: "var(--ui-font-sans)", cursor: "pointer", background: `linear-gradient(135deg, ${accent}, var(--accent-strong))`, border: "none", color: "var(--text-on-accent)", fontWeight: 700, boxShadow: "var(--shadow-sm)", transition: "all 0.15s", display:"inline-flex", alignItems:"center", gap:6 }}><Save size={13} strokeWidth={2} /> Save</button>
              <button className="iaction-btn" onClick={handleDownload} style={{ padding: "6px 16px", borderRadius: 8, fontSize: 11, fontFamily: "var(--ui-font-sans)", cursor: "pointer", background: "var(--surface-2)", border: "1.5px solid var(--border)", color: "var(--info)", fontWeight: 600, transition: "all 0.15s" }}>↓ Export Today</button>
            </>
          )}
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflow: "hidden", display: "flex", flexDirection: "column" }}>
        ...
        {viewTab === "entry" && (
          <div style={{ flex: 1, overflow: "auto", padding: "20px 28px" }} className="ifade-in">
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16, padding: "14px 20px", background: "var(--card)", border: `1.5px solid ${accent}40`, borderRadius: 12, borderLeft: `5px solid ${accent}`, boxShadow: "0 2px 8px #10b98110" }}>
              <div>
            ...
            <div style={{ overflowX: "auto", background: "var(--card)", border: "1.5px solid #d1fae5", borderRadius: 12, overflow: "hidden", boxShadow: "0 2px 12px #10b98108" }}>
        ...
        {viewTab === "records" && (
          <div style={{ flex: 1, overflow: "auto", padding: "20px 28px" }} className="ifade-in">
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20, padding: "14px 20px", background: "var(--card)", border: "1.5px solid #d1fae5", borderRadius: 12, flexWrap: "wrap", boxShadow: "0 2px 8px #10b98108" }}>
              ...
              {filterMode === "custom" && (
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginLeft: 8 }}>
                  <input type="date" value={customStart} onChange={e => setCustomStart(e.target.value)} style={{ background: "var(--bg)", border: "1.5px solid #a7f3d0", color: "#064e3b", padding: "6px 12px", borderRadius: 8, fontSize: 11, fontFamily: "var(--ui-font-sans)", outline: "none" }} />
                  <span style={{ color: "#9ca3af", fontSize: 10 }}>to</span>
                  <input type="date" value={customEnd} onChange={e => setCustomEnd(e.target.value)} style={{ background: "var(--bg)", border: "1.5px solid #a7f3d0", color: "#064e3b", padding: "6px 12px", borderRadius: 8, fontSize: 11, fontFamily: "var(--ui-font-sans)", outline: "none" }} />
                </div>
              )}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14, marginBottom: 20 }}>
              {[
                ...
                <div key={label} style={{ background: "var(--card)", border: "1.5px solid #d1fae5", borderTop: `4px solid ${col}`, borderRadius: 12, padding: "16px 18px", position: "relative", overflow: "hidden", boxShadow: "0 2px 8px #10b98108" }}>
            ...
            {filteredEntries.length === 0 ? (
              <div style={{ textAlign: "center", padding: 60, color: "#d1fae5", fontSize: 12, letterSpacing: "3px", background: "var(--card)", border: "1.5px solid #d1fae5", borderRadius: 12, fontFamily: "var(--ui-font-sans)", fontWeight: 700 }}>NO RECORDS FOUND FOR THIS PERIOD</div>
            ) : (
