...
// ─── Columns from screenshot: S.No. | Claim ID | Patient Name | DOA | UploadDate | Hospital | IntimationBy | Pri.HOS/Dr. | Remarks | AddedBy
const COLUMNS = [
  ...
  { key: "intimationBy",  label: "IntimationBy",    width: 150 },
...
const accent      = "#38bdf8"; // sky blue — Intimation dept color
...
export default function IntimationDashboard({ currentUser, onLogout }) {
  ...
  const handleDownload = () => {
    ...
    const wsData = [
      ["SANGI INTIMATION", "", "", "", "", "", "", "", "", ""],
      ["S.No.", "Claim ID", "Patient Name", "DOA", "UploadDate", "Hospital", "IntimationBy", "Pri.HOS/Dr.", "Remarks", "AddedBy"],
      ...data.map((r, i) => [i + 1, r.claimId, r.patientName, r.doa, r.uploadDate, r.hospital, r.intimationBy, r.priHosdr, r.remarks, r.addedBy]),
    ];
    const ws = XLSX.utils.aoa_to_sheet(wsData);
    ws["!cols"] = [8, 15, 22, 14, 14, 20, 18, 18, 24, 16].map(w => ({ wch: w }));
    ws["!merges"] = [{ s: { r: 0, c: 0 }, e: { r: 0, c: 9 } }];
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Intimation Log");
    const label = filterMode === "custom" ? `${customStart}_${customEnd}` : filterMode;
    XLSX.writeFile(wb, `Sangi_Intimation_${label}_${today}.xlsx`);
  };
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100vh", background: "#060a10", color: "#e2e8f0", fontFamily: "'IBM Plex Mono','Courier New',monospace", overflow: "hidden" }}>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .igrid-cell:focus { outline: 2px solid ${accent}; outline-offset: -2px; background: #001520 !important; z-index: 2; position: relative; }
        .igrid-cell { transition: background 0.1s; }
        .igrid-cell:hover { background: #000f18 !important; }
        .itab-btn:hover { color: #e2e8f0 !important; }
        .ifilter-chip:hover { border-color: ${accent} !important; color: ${accent} !important; }
        .iaction-btn:hover { filter: brightness(1.15); }
        .irow-remove { opacity: 0; transition: opacity 0.15s; }
        tr:hover .irow-remove { opacity: 1; }
        @keyframes ifadeIn { from { opacity: 0; transform: translateY(6px); } to { opacity: 1; transform: none; } }
        .ifade-in { animation: ifadeIn 0.3s ease; }
        @keyframes ipulse { 0%,100%{opacity:1}50%{opacity:.35} }
      `}</style>
      ...
      <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 28px", height: 58, borderBottom: "1px solid #16202e", background: "#0b1018", flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 15, fontWeight: 700, color: "#f1f5f9" }}>Intimation Department</div>
          </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          {currentUser && (
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginLeft: 8, paddingLeft: 12, borderLeft: "1px solid #16202e" }}>
              <div style={{ width: 28, height: 28, borderRadius: 7, background: `${accent}20`, border: `1px solid ${accent}40`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, color: accent, fontWeight: 700 }}>{currentUser.name?.[0] || "I"}</div>
              <div>
                <div style={{ fontSize: 11, color: "#e2e8f0", fontWeight: 600 }}>{currentUser.name}</div>
                <div style={{ fontSize: 9, color: "#374151", letterSpacing: "1px", textTransform: "uppercase" }}>Intimation Staff</div>
              </div>
        ...
        <div style={{ display: "flex", gap: 4 }}>
          {[{ id: "entry", label: "📋 Daily Entry" }, { id: "records", label: "🗂 Records & Export" }].map(tab => (
            <button key={tab.id} className="itab-btn" onClick={() => setViewTab(tab.id)}
              style={{ padding: "6px 18px", borderRadius: "6px 6px 0 0", fontSize: 11, fontFamily: "inherit", cursor: "pointer", border: "none", background: viewTab === tab.id ? "#001a2a" : "transparent", color: viewTab === tab.id ? accent : "#374151", borderBottom: viewTab === tab.id ? `2px solid ${accent}` : "2px solid transparent", letterSpacing: "0.5px", transition: "all 0.15s" }}>
              {tab.label}
            </button>
          ))}
        </div>
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          {viewTab === "entry" && (
            <>
              ...
              {hasUnsaved && <div style={{ fontSize: 10, color: "#f59e0b", animation: "ipulse 2s infinite" }}>● Unsaved changes</div>}
              {savedAt && !hasUnsaved && <div style={{ fontSize: 10, color: "#34d399" }}>✓ Saved at {savedAt}</div>}
              <button className="iaction-btn" onClick={() => addRows(5)} style={{ padding: "5px 14px", borderRadius: 6, fontSize: 10, fontFamily: "inherit", cursor: "pointer", background: "#0f172a", border: "1px solid #1e2a3a", color: "#64748b" }}>+ 5 Rows</button>
              <button className="iaction-btn" onClick={handleSave} style={{ padding: "5px 16px", borderRadius: 6, fontSize: 11, fontFamily: "inherit", cursor: "pointer", background: accent, border: `1px solid ${accent}`, color: "#000", fontWeight: 700 }}>💾 Save</button>
              <button className="iaction-btn" onClick={handleDownload} style={{ padding: "5px 14px", borderRadius: 6, fontSize: 10, fontFamily: "inherit", cursor: "pointer", background: "#064e3b", border: "1px solid #065f46", color: "#34d399" }}>↓ Export Today</button>
            </>
          )}
          {viewTab === "records" && (
            <button className="iaction-btn" onClick={handleDownload} style={{ padding: "5px 18px", borderRadius: 6, fontSize: 11, fontFamily: "inherit", cursor: "pointer", background: "#064e3b", border: "1px solid #065f46", color: "#34d399", fontWeight: 700 }}>↓ Download Excel</button>
          )}
        </div>
      </div>
        ...
        {viewTab === "entry" && (
          <div style={{ flex: 1, overflow: "auto", padding: "20px 28px" }} className="ifade-in">
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16, padding: "12px 18px", background: "#0b1018", border: `1px solid ${accent}30`, borderRadius: 10, borderLeft: `4px solid ${accent}` }}>
              <div>
                <div style={{ fontSize: 12, fontWeight: 700, color: "#f1f5f9" }}>Today's Intimation Log — {today}</div>
                    ...
                    return (
                      <tr key={row.id} style={{ background: filled ? "#0a0f1a" : "transparent", borderBottom: "1px solid #0d1520" }}>
                        ...
                        <td style={{ padding: "0 8px", borderRight: "1px solid #0d1520", textAlign: "center" }}>
                          <button className="irow-remove" onClick={() => removeRow(row.id)} style={{ background: "none", border: "none", color: "#ef4444", cursor: "pointer", fontSize: 14, padding: "0 4px", fontFamily: "inherit" }}>✕</button>
                        </td>
                      </tr>
                    );
                  })}
                    ...
                    <tbody>
                      {filteredEntries.map((row, i) => (
                        <tr key={row.id || i} style={{ borderBottom: "1px solid #0d1520" }}>
                          ...
                          <td style={{ padding: "10px 14px", color: "#94a3b8", borderRight: "1px solid #0d1520" }}>{row.intimationBy || "—"}</td>
                          <td style={{ padding: "10px 14px", color: "#94a3b8", borderRight: "1px solid #0d1520" }}>{row.priHosdr || "—"}</td>
