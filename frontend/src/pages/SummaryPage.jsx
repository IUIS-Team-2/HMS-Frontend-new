...
import { fmtDT, maskAadhaar } from "../utils/helpers";
...
export default function SummaryPage({uhid,patient,discharge,svcs,billing,locId,admNo,onPrint,onRequestPrint,branch}){
  const total=svcs.reduce((a,s)=>a+(parseFloat(s.rate)||0)*(parseFloat(s.qty)||0),0);
 ...
 return(
  ...
  >
    ...
    <Card icon={IC.person} title="Patient Information" delay={0}><div className="g2">{[["UHID",uhid],["Admission #",`#${admNo}`],["Patient Name",patient.patientName],["Guardian",patient.guardianName],["Gender",patient.gender],["Blood Group",patient.bloodGroup],["Phone",patient.phone],["National ID", maskAadhaar(patient.nationalId)],["Branch",`${loc.name}, Mathura`],["Address",patient.address]].map(([l,v])=>(<div key={l}><div style={{fontSize:11,fontWeight:600,color:T.textMuted,textTransform:"uppercase",letterSpacing:".05em",marginBottom:3}}>{l}</div><div style={{fontSize:14,fontWeight:500,color:T.text}}>{v||"—"}</div></div>))}</div></Card>
