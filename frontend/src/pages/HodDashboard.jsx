...
export default function HodDashboard({ currentUser, onLogout }) {
  const renderIcon = (Icon, size = 15, sw = 2) => (Icon ? <Icon size={size} strokeWidth={sw} /> : null);
  ...
  const s = {
    ...
    deptBtn: (active, dept) => ({
      ...
      color: active ? DEPT_COLORS[dept] : "var(--text-muted)",
      fontSize: "12px",
      letterSpacing: "0.5px",
      transition: "all 0.15s",
      border: "none",
    ...
    btn: (variant = "default") => ({
      ...
      ...(variant === "primary"
        ...
        : { background: "var(--surface-2)", borderColor: "var(--border-strong)", color: "var(--text-muted)" }),
    }),
    content: {
      flex: 1,
      overflowY: "auto",
    ...
    badge: (status) => ({
      ...
      fontSize: "10px",
      letterSpacing: "0.5px",
      background: STATUS_COLORS[status]?.bg || "var(--border-strong)",
      color: STATUS_COLORS[status]?.text || "var(--text-mid)",
      border: `1px solid ${STATUS_COLORS[status]?.border || "var(--border-strong)"}`,
    }),
    priorityDot: (p) => ({
      display: "inline-block",
      width: "7px",
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((task) => (
                  <tr key={task.id} style={{ transition: "background 0.1s" }}>
                    ...
                    <td style={{ ...s.td, color: "var(--text)", fontWeight: 600 }}>{task.employeeName || task.employeeId}</td>
                    <td style={s.td}>{task.taskType}</td>
                    <td style={{ ...s.td, color: "var(--text)" }}>{task.patientId || "—"}</td>
                    <td style={s.td}>
                      {task.patientType && (
    ...
    return (
      <>
        ...
        {analytics ? (
          ...
          <div style={{ ...s.emptyRow, padding: 60, color: "var(--border-strong)" }}>LOADING ANALYTICS...</div>
        )}
      </>
    );
  }
            ...
            employees.map((emp, i) => (
              <div key={emp.id} style={s.empCard}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 10, color: "var(--text-muted)", letterSpacing: "1px", marginTop: 2 }}>{emp.role || "Staff"}</div>
      ...
      <aside style={s.sidebar}>
        ...
        {[
          ...
          <button key={v.id} style={s.navBtn(activeView === v.id)} onClick={() => setActiveView(v.id)}>
            <span style={s.navIcon(activeView === v.id)}>{renderIcon(VIEW_ICONS[v.id], 14, 2)}</span>
