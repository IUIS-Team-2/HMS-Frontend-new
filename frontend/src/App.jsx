...
export default function App() {
  ...
  const handleLogout = () => {
    setLoggedIn(false); setCurrentUser(null); resetAll(); setPrintRequests([]);
    try {
      sessionStorage.removeItem('hms_loggedIn'); sessionStorage.removeItem('hms_currentUser');
      sessionStorage.removeItem('hms_page');     sessionStorage.removeItem('hms_token');
    } catch {}
  };
  ...
  const loadDashboardData = async (userRole) => {
          ...
          p.admissions.forEach(adm => {
            if (adm.billing && adm.billing.printStatus === 'PENDING') {
              formattedRequests.push({
                ...
                patient: p, adm, svcs: adm.services || [],
    ...
    } catch (error) { console.error("Failed to load live backend data:", error); }
  };
 
  useEffect(() => {
    if (loggedIn && currentUser) loadDashboardData(currentUser.role);
  ...
  const resetAll = () => {
    ...
    setPatient(blankPatient()); setDischarge(blankDischarge()); setSvcs([]); setBilling(blankBilling()); setErrs({});
  ...
  const handleNewAdmission = (existing) => {
    ...
    setPatient(pd); setUhid(existing.uhid); setIsReturning(true); setSubPage("form");
  };
 
  const handleDischargeFromHistory = (patientObj, admObj) => {
    const { admissions, ...pd } = patientObj;
    setPatient(pd); setUhid(patientObj.uhid); setAdmNo(admObj.admNo); setIsReturning(true);
    const doaValue = admObj.discharge?.doa || admObj.dateTime || "";
    setDischarge({ ...blankDischarge(), ...(admObj.discharge || {}), doa: doaValue });
    setSvcs(admObj.services?.length ? admObj.services : []);
  ...
  const handleSetExpectedDod = async (uhidToUpdate, admNoToUpdate, date) => {
    ...
    } catch { toast.error("Failed to set Expected Discharge Date on server."); }
  };
  ...
  const handleRegister = async () => {
    ...
    try {
      if (isReturning && uhid) {
        ...
        setAdmNo(admResponse.admNo); setShowUHID(true); setSubPage("search");
        const localNow = new Date(); localNow.setMinutes(localNow.getMinutes() - localNow.getTimezoneOffset());
    ...
    } catch { toast.error("Error registering patient. Check console."); }
  };
  ...
  const handleSaveMedical = async () => {
    try {
      ...
      setMedicalDone(true); setPage("discharge"); toast.success("Medical History saved!");
  ...
  const handleSaveDischarge = async () => {
    try {
      ...
      setDischargeDone(true); setPage("services"); toast.success("Discharge details saved!");
  ...
  const handleRequestPrint = async () => {
    try { await apiService.requestPrint(uhid, admNo); toast.success("Print request sent to Super Admin!"); }
    catch { toast.error("Failed to send print request."); }
  };
  ...
  if (page === "superadmin") {
    return (
      <>
        ...
        <ThemeProvider>
          <SuperAdminDashboard db={db} printRequests={printRequests} onApprovePrint={handleApprovePrint} onViewBill={handleViewBill} onLogout={handleLogout} />
  ...
  if (page === "managementadmin") {
    return (
      <ThemeProvider>
        <ManagementAdminDashboard currentUser={currentUser} db={db} onLogout={handleLogout} />
      </ThemeProvider>
    );
  }
          ...
          {currentUser && (
            <div style={{ display:"flex", alignItems:"center", gap:10, marginLeft:12 }}>
              <div style={{ fontSize:12, lineHeight:1.4, textAlign:"right" }}>
                ...
                <div style={{ color:"rgba(255,255,255,.5)" }}>{locId === "laxmi" ? "Laxmi Nagar Branch" : "Raya Branch"}</div>
              </div>
              <button onClick={handleLogout} style={{ padding:"6px 14px", borderRadius:8, background:"rgba(255,255,255,.1)", border:"1px solid rgba(255,255,255,.2)", color:"#fff", fontSize:12, fontWeight:600, cursor:"pointer" }}>Logout</button>
            </div>
          )}
        </div>
      </header>
              ...
              return (
                <div key={p.id} className={`nav-item${active?" active":""}${done&&!active?" done":""}${locked?" locked":""}`} onClick={() => navTo(p.id)}>
                  ...
                  <span className="nav-step-num">
                    {p.id === "medical" && !medicalDone && patientDone ? <span style={{ fontSize:9, color:T.amber }}>!</span> : done ? <Ico d={IC.check} size={10} sw={2.5} /> : i+1}
                  </span>
                </div>
              );
            })}
...
export function useAuth() {
  ...
  const logout = () => { sessionStorage.removeItem('hms_loggedIn'); sessionStorage.removeItem('hms_currentUser'); sessionStorage.removeItem('hms_page'); sessionStorage.removeItem('hms_token'); window.location.reload(); };
  return { user, logout };
}
